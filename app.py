...
# Row 1: Retention rate trend and satisfaction scores
...
with col1:
    ...
    for term, color, marker in [("Spring", BLUE, "o"), ("Fall", ORANGE, "s")]:
        sub = ret_data[ret_data["Term"] == term]
        if not sub.empty:
            ax.plot(sub["Year"], sub["Retention Rate (%)"], color=color,
                    marker=marker, linewidth=2, markersize=6, label=term)
    ax.set_xlabel("Year")
...
# Row 2: Spring vs Fall comparison and department pie chart
...
# Row 3: Department enrollment trends over time
st.markdown("#### Department Enrollment Trends")
dept_cols_trend = [f"{d} Enrolled" for d in departments if f"{d} Enrolled" in fdf.columns]
dept_trend = fdf.groupby("Year")[dept_cols_trend].sum().reset_index()
